...
export default function ImportPage() {
  ...
  // Anime doucement la barre vers la cible sans dépasser
  const animateTo = (target: number) => {
    if (animFrameRef.current) clearInterval(animFrameRef.current)
    animFrameRef.current = setInterval(() => {
      setProgress(prev => {
        if (prev >= target) { clearInterval(animFrameRef.current!); return target }
        return Math.min(prev + 0.8, target)
      })
    }, 40)
  }

  useEffect(() => () => { if (animFrameRef.current) clearInterval(animFrameRef.current) }, [])

  const setStageAndAnimate = (s: Stage) => {
    setStage(s)
  ...
  const handleSubmit = async () => {
    ...
    setStageAndAnimate('reading')
    await new Promise(r => setTimeout(r, 300))

    setStageAndAnimate('sending')
    const form = new FormData()
    form.append('releve', file)

    // Passer à "analysing" après 1s (Claude démarre le traitement)
    const analysingTimer = setTimeout(() => setStageAndAnimate('analysing'), 1000)
    ...
    try {
      ...
      setStageAndAnimate('saving')
      await new Promise(r => setTimeout(r, 400))
      setStageAndAnimate('done')
      setMessage(`${data.releve.transactions.length} transactions importées.`)
      setTimeout(() => router.push(`/?releve=${data.releve.id}`), 1500)
    } catch {
      clearTimeout(analysingTimer)
      setStage('error')
      setProgress(0)
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 flex flex-col">
        <div className="bg-white border-b border-[#ebebeb] px-7 py-3.5">
          <h1 className="text-[16px] font-bold tracking-[-0.3px]">Importer un relevé</h1>
        </div>
          ...
          <div className="w-full max-w-lg">
            <div className="bg-white border border-[#ebebeb] rounded-2xl p-8 shadow-sm">
              ...
              <p className="text-[13px] text-[#999] mb-7">
                Déposez votre relevé PDF Crédit Agricole. Claude extrait et catégorise toutes les transactions.
              </p>
              ...
              >
                ...
                {isRunning ? (
                  ...
                  <>
                    ...
                    <p className="text-[12px] text-[#999] mt-1">{message}</p>
                  </>
                ) : file ? (
                  <>
                    <div className="text-3xl mb-3">📄</div>
                  ...
                  <>
                    <div className="text-3xl mb-3">🏦</div>
                    <p className="text-[14px] font-semibold text-[#111]">Relevé bancaire PDF</p>
                    <p className="text-[12px] text-[#999] mt-1">Glisser-déposer ou cliquer pour parcourir</p>
                    <p className="text-[11px] text-[#ccc] mt-2">Crédit Agricole · PDF uniquement</p>
                  </>
                )}
              </div>
              ...
              {(isRunning || stage === 'done') && (
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[12px] font-semibold tabular-nums" style={{ color: '#00b37e' }}>
                      {Math.round(progress)}%
                    </span>
                  </div>
                  <div className="h-1.5 bg-[#f2f2f2] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${progress}%`, background: '#00b37e' }}
                    />
                  </div>
                  {/* Étapes */}
                    ...
                    {STAGES.filter(s => s.key !== 'done').map(s => (
                      <div key={s.key} className="flex flex-col items-center gap-1">
                        <div className={`w-2 h-2 rounded-full transition-colors ${
                          progress >= s.pct ? 'bg-[#00b37e]' : 'bg-[#e5e5e5]'
                        }`} />
                        <span className="text-[10px] text-[#bbb] whitespace-nowrap">
                          {s.label.replace('…', '')}
                        </span>
                      </div>
                    ))}
              ...
              >
                {isRunning ? currentLabel : stage === 'done' ? '✅ Redirection…' : '✦ Analyser avec Claude'}
              </button>

              {!isRunning && stage === 'idle' && (
                <p className="text-center text-[11px] text-[#ccc] mt-4">
                  Traitement local · ~15–30 secondes
                </p>
              )}
            </div>
          </div>
